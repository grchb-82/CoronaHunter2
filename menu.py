...
import sys
from text_image import QuitGUI,ResumeGUI,FullscreenGUI,SettingsGUI, ResolutionGUI, SelectorGui
from config   import CONFIG, compute_flags, save_config
from settings import WIDTH, HEIGHT

def Title_Screen(Title,game_surface,screen,Version):
...
def Pause_Menu(gui_elements,player,gs,shot_sound, fanfare_sound,screen ):
    ...
    quit = QuitGUI(gui_x, gui_y_increment * 2 + gui_y)
    settings = SettingsGUI(gui_x, gui_y_increment * 3 + gui_y)
    fullscreen = FullscreenGUI(gui_x, gui_y_increment * 4 + gui_y)
    resolution = ResolutionGUI(gui_x, gui_y_increment * 5 + gui_y)
    selector = SelectorGui(gui_x, gui_y_increment * gs.selector_position + gui_y)
    gui_elements.add(resume, quit, fullscreen,settings, resolution, selector)

    if player.UP_KEY and gs.selector_position > 1 and gs.selector_position <=4:
        gs.selector_position -= 1
        if gs.selector_position == 3:
            gs.selector_position -= 1
        #print("selector_position",gs.selector_position)
        player.UP_KEY = False
    elif player.DOWN_KEY and gs.selector_position >= 1 and gs.selector_position < 4:
        gs.selector_position += 1
        if gs.selector_position == 3:
            gs.selector_position += 1
        #print("selector_position",gs.selector_position)
